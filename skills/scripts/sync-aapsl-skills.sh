#!/usr/bin/env bash
# Librarian helper — compare vendor/aap-skills-library skills with skills/platform/aap-*.
# Does not overwrite enterprise adaptations automatically.
# Usage:
#   ./skills/scripts/sync-aapsl-skills.sh --diff    # show upstream vs platform diffs
#   ./skills/scripts/sync-aapsl-skills.sh           # same as --diff (safe default)
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
VENDOR="${ROOT}/skills/vendor/aap-skills-library/skills"
PLATFORM="${ROOT}/skills/platform"
MODE="diff"
...
for vendor_skill in "${VENDOR}"/aap-*/SKILL.md; do
  ...
  if [[ ! -f "${platform_skill}" ]]; then
    echo "[MISSING] ${name} — in vendor, not yet in skills/platform/"
    continue
  fi
...
echo "Done. Librarian merges vendor changes into skills/platform/ manually after white book review."
